...
if (airline::airline_name_exists($_GET["name"])) {
    ...
    while (isset($_GET["Speclug" . $counter])) {
        ...
        if ($spec != null) {
            $specialeBagage[] = SpecialLuggage::GetCombo($airline->airline_id, $spec->
                specialluggage_id);
...
<html>
    ...
    <div id="container">
            <div id="content">
            <div id="airLogo"><img style="width: 175px; height:175px; " src="images/airlines/<?php echo
$airline->logo; ?>" /></div>
          <hr/>
...
echo htmlspecialchars($airline->name); ?>
            ...
            <td><?php if ($airline->classes[0]->maxWeightLuggage != 0) {
    echo $airline->classes[0]->maxWeightLuggage . 'kg';
} else {
    echo 'n.v.t.';
} ?></td>
           <td>Gratis aantal handbagage:</td>
            <td><?php if ($airline->classes[0]->pcsHL != 0) {
    echo $airline->classes[0]->pcsHL;
} else {
    echo 'n.v.t..';
} ?></td>
            ...
            <td><?php if ($airline->classes[0]->pcsLuggage != 0) {
    echo $airline->classes[0]->pcsLuggage;
} else {
    echo 'n.v.t.';
} ?></td>
            <td>Gratis gewicht handbagage:</td>
            <td><?php if ($airline->classes[0]->MaxWeightHL != 0) {
    echo $airline->classes[0]->MaxWeightHL . 'kg';
} else {
    echo 'n.v.t.';
} ?></td>
            </tr>
            ...
            <td><?php $afmetingen = $airline->classes[0]->sizeTotalPerItem ? $airline->
classes[0]->sizeTotalPerItem . 'cm' : $airline->classes[0]->sizeLenghtPerItem .
    ' x ' . $airline->classes[0]->sizeWidthPerItem . ' x ' . $airline->classes[0]->
    sizeHeightPerItem . 'cm';
echo $afmetingen;
?></td>
            <td>Handbagage afmetingen:</td>
            <td><?php if ($airline->classes[0]->sizeTotalHL != 0) {
    echo $airline->classes[0]->sizeTotalHL . 'cm';
} else {
    $afmetingen = $airline->classes[0]->sizeLenghtHL . ' x ' . $airline->classes[0]->
        SizeWidthHL . ' x ' . $airline->classes[0]->sizeHeightHL . 'cm';
    echo $afmetingen;
} ?></td>
            ...
            <td>&euro;<?php echo number_format($airline->OverweightChargeG, 2,
',', ' '); ?></td>
                 <td>Kosten te grote koffer:</td>
            <td>&euro;<?php echo number_format($airline->OversizeCharge, 2, ',',
' '); ?></td>
            </tr>
             <tr>
             <td>Kosten extra koffer:</td>
            <td><?php foreach ($airline->ChargeExtraBag as $bag) {
    echo 'koffer ' . ($bag->number + 1) . ': &euro;' . number_format($bag->costs, 2,
        ',', ' ') . '</br>';
} ?></td>
                 <td>Kosten overgewicht per koffer:</td>
            <td>&euro;<?php echo number_format($airline->OverweightChargeBag, 2,
',', ' '); ?></td>
            </tr>
...
?>
    ...
    foreach ($specialeBagage as $specbag) {
        ...
        echo '<tr><td>Naam: ' . $naam . '</td><td>Prijs: &euro;' . number_format($fare,
            2, ',', ' ') . '<br/>Max. gewicht(kg):' . $weight . '<br/>Dimensies:' . $dimension .
            '<br/>Opmerkingen:' . $notes . '</td></tr>';
    } ?>
            </table>
                    </div>
                    <hr />
            ...
            <td>
                        <?php echo $airline->classes[0]->
pcsLuggageInfantMaxWeight; ?>kg</td></tr>
                        <tr>
                        <td>Stukken handbagage  kind:</td><td><?php echo $airline->
classes[0]->pcsInfantHL; ?></td>
                        <td>Max. gewicht handbagage  kind:</td><td><?php echo $airline->
classes[0]->MaxWeightInfantHL; ?>kg</td>
                        </tr>
            </table>
            ...
            <td>
            <?php if ($airline->classes[0]->LaptopAllowedHL == true) {
    echo 'ja';
} else {
    echo 'nee';
}
; ?>
            ...
            <td>
            <?php if ($airline->classes[0]->Pooling == true) {
    echo 'ja';
} else {
    echo 'nee';
}
; ?>
            ...
            <td>
            <?php if ($airline->classes[0]->FreeWheelChair == true) {
    echo 'ja';
} else {
    echo 'nee';
}
; ?>
            ...
            <td>
            <?php if ($airline->classes[0]->FreeServiceDog == true) {
    echo 'ja';
} else {
    echo 'nee';
}
; ?>
            ...
            <td> Waarde aangifte toegestaan:
                ...
                <td><?php if ($airline->classes[0]->DeclarationOfValue == true) {
    echo 'ja';
} else {
    echo 'nee';
} ?></td>
                ...
                <td>&euro;
                <?php echo number_format($airline->classes[0]->
MaxDeclarationOfValue, 2, ',', ' '); ?>
                </td>
                    <?php } ?>
                </tr>
                <tr>
                ...
                <td><?php if ($airline->classes[0]->LoyaltyProgramme == true) {
    echo 'ja';
} else {
    echo 'nee';
} ?></td>
