...
class chargeExtraBag
{
    ...
    {
        DbHandler::NonQuery("UPDATE `chargeextrabag` SET `airline` = :airline, `number` = :number, `costs` = :costs WHERE `ChargeExtraBag_id` = :id",
            array(
            "airline" => $charge->airline,
            "number" => $charge->number,
            "costs" => $charge->costs,
            "id" => $charge->chargeExtraBag_id));
    }
    //een toevoegen
    public static function add($airline_id, $number, $costs)
    {
        DbHandler::NonQuery("INSERT INTO `chargeextrabag` (`airline`, `number`, `costs`) VALUES (:airline, :number, :costs)",
            array(
            "airline" => $airline_id,
            "number" => $number,
            "costs" => $costs));
    ...
    {
        DbHandler::NonQuery("DELETE FROM `chargeextrabag` WHERE `airline` = :airline",
