...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["naam"], $_POST["mail"], $_POST["onderwerp"], $_POST["bericht"],
        $_POST["code"]) && !empty($_POST["naam"]) && !empty($_POST["mail"]) && !empty($_POST["mail"]) &&
        !empty($_POST["bericht"]) && !empty($_POST["code"])) {
        ...
        if (validator::isEmail($_POST["mail"]) && validator::stringLimit(100, 100000, $_POST["bericht"]) &&
            validator::validateCaptcha($_POST["code"], $_SESSION["hash"])) {
            mail(ADMIN_EMAIL, "Contactformulier vliegbagage: " . $_POST["onderwerp"], $_POST["bericht"],
                "Reply-To: " . $_POST["mail"]);
        } else {
...
?>
            ...
            Naam:<br /><input type="text" name="naam" value="<?php echo (isset($_POST["naam"]) ?
htmlspecialchars($_POST["naam"]) : ""); ?>" /><br />
            Uw emailadres: <?php echo (isset($error["mail"]) ? $error["mail"] :
""); ?><br /><input type="email" name="mail" value="<?php echo (isset
($_POST["mail"]) ? htmlspecialchars($_POST["mail"]) : ""); ?>"/><br />
            Onderwerp:<br /><input type="text" name="onderwerp" value="<?php echo (isset
($_POST["onderwerp"]) ? htmlspecialchars($_POST["onderwerp"]) : ""); ?>" /><br />
            Uw vraag: <?php echo (isset($error["bericht"]) ? $error["bericht"] :
""); ?><br /><textarea name="bericht" rows="10" cols="30"><?php echo (isset
($_POST["bericht"]) ? htmlspecialchars($_POST["bericht"]) : ""); ?></textarea>
            <br /><br />
            <img src="captcha/captcha.php" alt="Code"/><br />
            Vul de bovenstaande code in: <?php echo (isset($error["code"]) ? $error["code"] :
"") ?><br />
